...
"#,
    ...
    async fn direct_handler_returns_metadata_and_redacted_payload_on_block() {
        let core = Arc::new(
            SecretsDetectionCore::new(plugin_config(json!({
        ...
        let handler = StageHandler::new(core, Stage::ToolPreInvoke);
        let payload = tool_call_payload(HashMap::from([(
            "message".to_string(),
            json!("AWS_ACCESS_KEY_ID=AKIAFAKE12345EXAMPLE"),
        )]));
        ...
        let result = handler
            .handle(&payload, &extensions_with_trace("trace-1"), &mut ctx)
            .await;

        assert!(!result.continue_processing);
        assert_eq!(result.violation.as_ref().unwrap().code, VIOLATION_CODE);
        assert_eq!(
            result.metadata.as_ref().unwrap()["secrets_detection"]["total_blocked"],
            json!(1)
        );
        assert_eq!(
