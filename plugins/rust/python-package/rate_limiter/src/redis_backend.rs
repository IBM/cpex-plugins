...
impl RedisTlsConfig {
    ...
    pub fn validate_and_build(
        ...
        // native-tls build: the redis crate's native-tls path uses the host OS
        // trust store and does NOT expose build_with_tls / TlsCertificates, so
        // explicit CA / mTLS / insecure knobs cannot be honored here. If any are
        // set, warn and fall through to Client::open(redis_url), which connects
        // via native-tls (OpenSSL on Linux) against the system trust store.
        let _ = redis_url;
        if self.ca_certs_path.is_some()
            || self.certfile_path.is_some()
            || self.keyfile_path.is_some()
            || !self.check_hostname
        {
            log::warn!(
                "rate limiter (native-tls build): redis_ssl_* options are ignored; \
                 native-tls uses the host OS trust store, so ensure the server CA is \
                 installed there (e.g. via ca-certificates)."
            );
        }
        // Always use the plain Client::open path (native-tls + system trust store).
        Ok(None)
    }
}
